...
| 6 | Системный промпт HomeGuru | ⏳ Pending | - |
...
**TDD Планирование:**
- [ ] Согласовать архитектуру: Config загружает промпт из файла, CommandHandler получает дополнительную команду `/role`
- [ ] План тестов:
...
- [ ] 🔴 RED: Написать тест `test_config_loads_prompt_from_file()`
- [ ] 🟢 GREEN: Создать файл `src/bot/system_prompt.txt` с описанием роли HomeGuru
- [ ] 🔵 REFACTOR: Обновить `src/bot/config.py`: добавить метод `load_system_prompt()` для загрузки из файла
- [ ] Описать специализацию: дизайн интерьеров, консультации по стилям, анализ помещений
- [ ] Указать ключевые компетенции: цветовые решения, мебель, декор
- [ ] Определить стиль общения: профессиональный, дружелюбный, экспертный

**Обработка ошибок загрузки:**
- [ ] 🔴 RED: Написать тест `test_config_prompt_file_not_found()`
- [ ] 🟢 GREEN: Добавить обработку `FileNotFoundError` в Config
- [ ] 🔵 REFACTOR: Fallback на дефолтный промпт при отсутствии файла
- [ ] Удалить `SYSTEM_PROMPT` из `.env` (теперь в файле)
- [ ] Обновить `.env.example` (убрать SYSTEM_PROMPT)

**Команда /role:**
- [ ] 🔴 RED: Написать тест `test_command_handler_get_role_message()`
- [ ] 🟢 GREEN: Добавить метод `get_role_message()` в `CommandHandler`
- [ ] 🔵 REFACTOR: Добавить обработчик команды `/role` в `TelegramBot.cmd_role()`
- [ ] Отображение информации о роли HomeGuru
- [ ] Перечисление ключевых возможностей и специализаций
- [ ] Обновить метод `get_help_message()` в `CommandHandler` с информацией о `/role`

**Обновление приветствия:**
- [ ] 🔴 RED: Написать тест `test_start_message_with_homeguru_context()`
- [ ] 🟢 GREEN: Обновить `get_start_message()` в `CommandHandler` - приветствие от HomeGuru
- [ ] 🔵 REFACTOR: Добавить описание специализации на дизайне интерьеров
...
- [ ] Роль HomeGuru через системный промпт в отдельном файле
- [ ] Команда `/role` для отображения специализации
