...
└───────────────┬───────────────────────────────┬────────────────┘
                ...
                ▼                               ▼
    ┌───────────────────────┐     ┌────────────────────────────┐
    │  Обработка команд     │     │    Context Manager         │
    │  - /start → Привет    │     │  ┌──────────────────────┐  │
...
- Автоматическая очистка устаревшего контекста по таймауту
...
4. Для текстовых сообщений:
   - Context Manager извлекает историю диалога пользователя
...
- Максимальное количество сообщений в истории: 10 пар (вопрос-ответ)
...
- При каждом обращении обновляется временная метка последней активности
- Проверка устаревших контекстов выполняется при каждом запросе
...
**Митигация:** Автоматическая очистка неактивных контекстов; жесткие лимиты на размер истории; мониторинг использования памяти.

#### Риск: Невозможность масштабирования при росте
**Митигация:** Архитектура спроектирована так, чтобы миграция к БД требовала изменений только в модуле Context Manager.
