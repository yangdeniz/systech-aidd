...
- **DialogueManager** - управление контекстом диалогов в памяти (поддержка мультимодальных сообщений)
- **LLMClient** - интеграция с OpenRouter API (Vision API для фото)
- **MediaProcessor** - обработка фотографий интерьеров
- **Config** - управление конфигурацией из .env
- **Interfaces** - Protocol для Dependency Inversion (SOLID DIP)

**В разработке (Итерации 8-9):**
- **Audio Processing** - обработка голосовых сообщений (Speech-to-Text)
